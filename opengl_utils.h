...
#define ogl(x) do { \
	...
	int _err = glGetError(); \
	if (_err) { \
		fprintf(stderr, "GL Error %d at %d, %s", _err, __LINE__, __func__); \
		exit(-1); \
	} \
} while (0)
...
static inline void oglProgramLog(int pid) {
	...
	if (!log) {
		fprintf(stderr, "Failed to allocate memory for the program log");
		return;
	}
	glGetProgramInfoLog(pid, logLen, &realLen, log);
	fprintf(stderr, "program %d log %s", pid, log);
	free(log);
}
